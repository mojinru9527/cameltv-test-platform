...
## 2026-09-18 | Batch 256 | 跨批次常犯问题核对（两次全量审计对比）

- **变更**：SKILL.md 新增「跨批次常犯问题核对」节（Dev 开工前扫已知风险表、QA 报告固定三问、同一问题二次出现必须升级为自动检查、文档承诺必须配可执行校验）并加一条 Red Flag；`cameltv-bug-guard` SKILL.md 新增「⚠️ 未关闭的已知风险（审计基线 2026-09-18）」表与「出网/执行/凭据安全」铁律，PATTERNS.md 新增 S1–S8 模式详解；新增审计基线证据 `work-logs/reviews/2026-09-18-code-audit-baseline.md`。
- **动因**：对 main `10e69b36` 与 `96cd5b65` 两次全量审计对比发现 8 类问题跨 98 个提交仍然存在（SSRF、凭据按域名关键字外发、`shell=True`、路径未收敛、密钥派生分裂、dry-run 当沙箱、静默吞异常、RUF012）；说明流水线缺少"谁负责核对历史问题"的环节，需要把核对动作固化到 Dev/QA 的固定步骤里。
