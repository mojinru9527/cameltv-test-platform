...
- [ ] **硬门禁通过**: 后端 `ruff check app/ --select F821` + `python scripts/ci/quality_ratchet.py` + `pip-audit -r requirements.txt`；前端 `npm run typecheck && npm run lint && npm run build && npm run test:a11y:ci && npm run lighthouse:a11y` + `npm audit --omit=dev --audit-level=high` + `node scripts/ci/npm_audit_ratchet.mjs`
