...
### batch-109 — 接口用例生成「测试考虑点」全量固化（Batch 109 Leader 条件）

| ID | 内容 | 优先级 | 创建日期 |
|----|------|--------|---------|
| C109-1 | 知识中心 capture 409 去重误判修复（C102-2 落地）后，将 `tests/test-case-standards/接口测试考虑点【辅助作用】.md` 导入生产知识中心（sportsadmin，项目 1），导入后 sources 列表可见 | P1 | 2026-08-06 |
| C107-2 | 接口关联能力（依赖接口/前置接口配置）上线后，场景测试模板从「待关联」升级为真实多接口串联用例 | P2 | 2026-08-06 |
