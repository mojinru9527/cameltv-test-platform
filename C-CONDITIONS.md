...
| ID | 内容 | 优先级 | 创建日期 |
|----|------|--------|---------|
| C259-1 | B2-6 DoD 的「被隐藏页面仍可经**权限 + 搜索**访问」中，"搜索"一半不成立：平台当前没有全局搜索/命令面板（前端 `rg` 无命中）。本批已保证"隐藏 ≠ 删除"（每个可见菜单恰好出现一次，历史 URL 与权限保留，路由表核对 + 断言）。解除条件=新增全局搜索/命令面板（属新接口/新行为 → 完整批次），覆盖全部授权菜单 code，并补「被收起页面可经搜索直达」的 E2E | P2 | 2026-09-19 |
...
- **Open / 非关闭**: 46（rows=166, deferred=8；口径见 `audit-cconditions.ps1` stats 输出，**2026-09-23 Batch 271 后**：44 + 新增 2 = 46）
- **In Progress**: 0
