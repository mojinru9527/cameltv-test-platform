...
| C68-4 | 正式域名发布决策登记到交付清单（本批演练已 200；域名启用/自定义域名决策由用户确认） | P1 | 2026-08-03（batch-69 已登记演练结论，决策项待用户） |

### batch-69 — AI 验收跟进修复（Batch 69 Leader 条件）

| ID | 内容 | 优先级 | 创建日期 |
|----|------|--------|---------|
| C69-1 | C68-1/J15 外部页 / J16 媒体授权到位后执行对应正负面验收；无授权保持 DEFERRED | P1 | 2026-08-03 |
| C69-2 | 正式域名发布决策（自定义域名/启用公告）由用户确认后登记关闭 C68-4 | P2 | 2026-08-03 |
| C69-3 | 分批生成耗时约 11 分钟（6 块），评估并发调用或降维提示优化；非阻塞 | P2 | 2026-08-03 |

### batch-70 — 能力产品化 UI 补齐（Batch 70 Leader 条件）

| ID | 内容 | 优先级 | 创建日期 |
|----|------|--------|---------|
| C70-1 | Playground 前端入口需 C22-C2/C3 runner 链路（真实编译+执行）验证通过后开放，否则维持 API-only | P2 | 2026-08-03 |
| C70-2 | 报告模板管理「设为默认」切换与章节级编辑（sections）可作后续增强；当前 CRUD 已满足产品化 | P2 | 2026-08-03 |
| C70-3 | 登录限流 10 次/15 分钟在自动化测试场景偏紧，评估按环境开关或提高阈值（非安全降级） | P1 | 2026-08-03 |
