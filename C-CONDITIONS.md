...
| C266-4 | **Web 用例在动态页上不稳定**：3 版本实跑 Web 通过数 29→26→21，失败集中在首页标签可见性断言（`News`/`BBall`/`Scores`/`Fixtures`/主标题）——目标站为异步渲染，标签出现时机不定；逐用例隔离已排除"上下文串味"，故属**用例稳定性**而非链路问题。解除条件=用例统一"先等到稳定锚点（如 h1 或主区块）再断言标签"，或用 `wait_visible` 带重试/更长超时，复跑 3 版使 Web 通过数稳定（如 ≥29/30 且无逐版下降） | P2 | 2026-09-19 |
