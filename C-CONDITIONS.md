...
| C103-4 | 全字段覆盖+真实可靠值原则：接口清单按功能需求确定；生产接口真实请求/响应优先回填基线；未上生产/测试中需求的接口同样以真实可靠值填写（测试环境回填或按功能需求+字段语义构造）；用例/调试数据须覆盖接口全部字段，按字段业务含义构造贴合数据（类型/含义/边界/枚举/格式/组合），禁止无意义 mock 占位值 | P1 | 2026-08-06 |
| C103-5 | 真实业务样本批量采集：Playwright 生产页面 XHR + 用户/业务提供，覆盖核心功能接口（首页/赛事/直播/我的/资讯/搜索等 ≥20 接口），按 C103-4 生成接口用例 | P1 | 2026-08-06 |
| C103-6 | AI 生成块级截断自动补全（C102-5 落地）：截断块补生成 + 覆盖缺口报告 | P2 | 2026-08-06 |
| C103-7 | 部署后回归新字段展示（请求参数/断言/请求结果三栏 + 正负向徽标）与执行回填链路 | P2 | 2026-08-06 |
...
| C114-2 | 交互 UI 自动化纳入每日回归（随 B112-3 UI 定时能力扩展一并落地） | P3 | 2026-08-07 |
