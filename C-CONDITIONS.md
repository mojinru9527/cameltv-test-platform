...
### batch-108 — capture 去重误判修复 + 规范导入闭环（Batch 108 Leader 条件）

| ID | 内容 | 优先级 | 创建日期 |
|----|------|--------|---------|
| C108-1 | Railway 部署环境增加 `KNOWLEDGE_INGEST_ENABLED=true` 后，复验生产 API capture：唯一内容→200+id、重复→409、开关关→503 明确提示；结果登记交付清单 | P1 | 2026-08-06 |
