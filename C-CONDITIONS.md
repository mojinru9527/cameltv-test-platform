...
| ID | 内容 | 优先级 | 创建日期 |
|----|------|--------|---------|
| C162-1 | 调度触发为同步执行：长计划（如 15.0.0 含 API 用例）触发时 HTTP 超网关返回 502（服务端继续执行 run#9）；应改为后台执行并立即返回 run_id | P2 | 2026-08-12 |
| C162-2 | 调度列表「执行环境」名称在页面挂载时未加载（仅打开表单时加载）→ 显示「环境#N」回退；应在挂载时加载 environments | P3 | 2026-08-12 |
