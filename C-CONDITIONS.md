...
**最后更新**: 2026-08-11 (Batch 146: 新增 C146-1~6 四视角审查修复条件)
...
| ID | 内容 | 优先级 | 创建日期 |
|----|------|--------|---------|
| C146-1 | 计划执行失败根因可见性修复：执行记录含 HTTP 状态/错误摘要/失败阶段；计划执行前置「环境/Token 就绪」检查（P0 TP-01，一键执行 325/325 失败） | P0 | 2026-08-11 |
...
- **Open / 非关闭**: 31 (含 5 个 P0 blocking；新增 C146-1~6 六项；口径见 `audit-cconditions.ps1` stats 输出)
- **In Progress**: 0
- **Closed**: 134（Batch 91 起以 `audit-cconditions.ps1` stats 输出为准）
- **Total**: 165（另有 13 条历史补录不计入）
