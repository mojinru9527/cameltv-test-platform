...
| HTTP 404 断言 | 20 | WARN（复核） | 隔离/权限/存在性守卫的 HTTP 404 是正确契约（不泄露存在性）；batch-86 起集中为 `assert_guard_404` 助手（21 处迁移） |
...
| 2026-08-04 | 86 | 209 | 0 | 21（404 断言集中为 assert_guard_404） | 消化迁移 + 基线刷新（Leader 复核） |
| 2026-08-05 | batch-89 | 209 | 0 | 0 | 自动审计 |
