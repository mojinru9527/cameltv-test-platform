...
## 3.1 复核记录（2026-09-19 推送前独立复演）

首次演练发生在本文档写出来**之前**（次序问题见文首）。本文档定稿后，按 §2 命令块**逐字**再执行一次，用来验证"照文档复演"这句话本身成立：

```
dump    : /opt/cameltv-backup/cameltv-prod-20260917-145752.dump
drill_db: restore_drill_20260919_132821
pg_restore_exit=0
tables=197
alembic=20260922_ai_agent_token
test_execution=137642
137650 | failed | 2026-09-17 04:00:00.552398
137649 | failed | 2026-09-17 04:00:00.552398
137648 | failed | 2026-09-17 04:00:00.552398
drop_exit=0
elapsed_seconds=20
...
**结论**：§2 命令块可逐字复演，结果与首次演练**逐项一致**（197 表 / 同一 alembic 版本 / 137642 行 / 同样 3 条执行记录 / 20 秒 / 临时库已 DROP）。

- **执行差异**：§2 中 `DUMP` 是占位符，复演时按注释取 `ls -t` 的最新一份；两次取到的都是 09-17 那份（09-18/09-19 无新 dump，见 §4 G2）。
- **磁盘水位**：复演前的只读检查（同日 13:25）`df -h /` = **74%**；首次记录为 73%，同一天两次读数差 1 个百分点（容器日志增长），均 < 80% 阈值。
