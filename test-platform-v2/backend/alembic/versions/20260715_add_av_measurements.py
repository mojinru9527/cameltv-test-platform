...
def upgrade() -> None:
    ...
    inspector = sa.inspect(op.get_bind())
    indexes = {index["name"] for index in inspector.get_indexes("av_check_measurement")}
    if "ix_av_check_measurement_task_id" not in indexes:
        op.create_index("ix_av_check_measurement_task_id", "av_check_measurement", ["task_id"])
    if "ix_av_check_measurement_metric_type" not in indexes:
...
def downgrade() -> None:
    ...
    if "av_check_measurement" not in inspector.get_table_names():
        return
    indexes = {index["name"] for index in inspector.get_indexes("av_check_measurement")}
    if "ix_av_check_measurement_metric_type" in indexes:
        op.drop_index("ix_av_check_measurement_metric_type", table_name="av_check_measurement")
    if "ix_av_check_measurement_task_id" in indexes:
