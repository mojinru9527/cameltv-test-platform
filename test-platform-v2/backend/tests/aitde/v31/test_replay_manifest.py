...
def test_manifest_refs_all_rows():
    ...
    steps = [
        ExecutionStep(
            ...
            step_type="API",
            status="SUCCEEDED",
            input_snapshot_json='{"method":"POST"}',
            output_snapshot_json='{"status":200}',
        )
    ]
    assertions = [
        AssertionResult(
            ...
            result="PASS",
            reason_code="eq",
            expected_json='{"status":200}',
            actual_json='{"status":200}',
        )
    ]
    evidence = [
        EvidenceArtifact(
    ...
    assert m["timeline"][0]["input_snapshot_json"] == {"method": "POST"}
    assert m["assertions"][0]["assertion_id"] == 20
    assert m["assertions"][0]["actual_json"] == {"status": 200}
    assert m["evidence"][0]["artifact_id"] == 30
    assert m["evidence"][0]["content_type"] == "application/json"
