...
const USERNAME = process.env.CAMELTV_USERNAME || '+86 18476944071';
const PASSWORD = process.env.CAMELTV_PASSWORD || 'a123456';
...
test.describe('CamelTv 生产环境冒烟测试', () => {
  ...
  test('TC-PROD-001: 首页加载正常', async ({ page }) => {
    ...
    // 截图留证
    await page.screenshot({ path: 'artifacts/prod-homepage.png', fullPage: false });
  });
  ...
  test('TC-PROD-002: 用户登录', async ({ page }) => {
    ...
    if (!currentUrl.includes('login')) {
      await page.goto(`${SITE_URL}/login`, { waitUntil: 'networkidle' }).catch(() => {
        // 某些站点登录可能是弹窗而非独立页面
      });
    }

    await page.screenshot({ path: 'artifacts/prod-login-page.png', fullPage: false });
    ...
    await page.screenshot({ path: 'artifacts/prod-after-login.png', fullPage: false });
    ...
    if (hasError) {
      ...
      await page.screenshot({ path: 'artifacts/prod-login-error.png', fullPage: true });
    }
  });
