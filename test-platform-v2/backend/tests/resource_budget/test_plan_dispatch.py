...
def test_pending_request_consumed_once_across_fresh_processes(tmp_path):
    ...
    with factory() as db:
        ...
        db.commit()
        job_id = queue.enqueue(
            db, plan_id=plan.id, project_id=1, executor_id=1, environment_id=None, ui_environment_id=None, auto_ui=False
        )
...
"""
    ...
    for _ in range(2):
        result = subprocess.run(
            [sys.executable, "-c", source, str(marker)], env=env, capture_output=True, text=True, timeout=30
        )
...
def test_http_job_status_and_missing_plan_contract(pending, db_session, client, auth_headers):
    ...
    db_session.commit()
    other = client.get(
        f"/api/v1/test-plans/{row.plan_id}/execution-jobs", headers={**auth_headers, "X-Project-Id": "2"}
    )
    assert other.json()["data"] == []
    missing = client.post("/api/v1/test-plans/99999/execute-all", headers=auth_headers, json={"async_mode": True})
    assert missing.status_code == 200 and missing.json()["code"] == 404


def test_async_submission_no_longer_creates_plan_job(pending, db_session, client, auth_headers, monkeypatch):
    row = db_session.get(PlanExecutionJob, pending)
    before = db_session.query(PlanExecutionJob).count()
    response = client.post(
        f"/api/v1/test-plans/{row.plan_id}/execute-all",
        headers=auth_headers,
        json={"async_mode": True, "auto_ui": False},
    )
    assert response.status_code == 200
    assert response.json()["code"] == 1
    assert db_session.query(PlanExecutionJob).count() == before
