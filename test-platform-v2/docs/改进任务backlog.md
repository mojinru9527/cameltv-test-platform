...
| B112-1 | ~~news/get 生产全 id 业务 400~~ → **Closed（口径 A）**：2026-08-07 用户确认详情页 SSR 正常打开；news/get 为内部/管理端端点，用户端以 SSR + get_visible 为准 | 生产多 id/头探测 + 用户页面实测 | 关闭（内部端点口径） | P1 |
...
> **标记**：2026-08-06 用户 Batch 102 验收反馈：AI 用例生成未对齐团队用例规范，用户端 92 功能点仅 77 条（约 0.84 条/FP），正负向/边界覆盖不足。

| # | 优化项 | 现状 | 目标 | 优先级 |
|---|--------|------|------|:----:|
| C1 | 规范对齐 | AI 生成提示词未注入 `tests/test-case-standards/`（等价类/边界值/场景法/错误推测） | 生成提示词注入规范 + case_design_method/positive_negative 字段 | P1 |
| C2 | 覆盖度 | 用户端 77 条（0.84 条/FP） | ≥2 条/FP（用户端 ≥184 条），正向/负向/边界齐全；覆盖缺口报告 | P1 |

## Epic API-VISIBILITY　接口用例参数/断言/结果可视（Batch 103 启动，C103-2）

> **标记**：2026-08-06 用户 Batch 102 验收反馈：接口用例看不到请求参数、断言与请求结果；且接口用例须按接口测试规范、以真实业务请求参数为基线生成（C103-2/C103-3）。

| # | 优化项 | 现状 | 目标 | 优先级 |
|---|--------|------|------|:----:|
| V1 | 请求参数可视 | 接口用例无入参展示 | request_params_json（header/query/body 字段+示例） | P1 |
| V2 | 断言可视 | 无断言字段 | assertions_json（状态码/字段/值匹配） | P1 |
| V3 | 请求结果可视 | 执行后无响应回填 | last_response_json + last_run_status，前端三栏展示 | P1 |
| V4 | 全字段语义覆盖+生产回填 | 无意义 mock 占位（测试偏移） | 接口清单按功能需求确定；生产接口真实请求/响应优先回填基线；用例/调试数据覆盖接口全部字段，按字段业务含义构造贴合数据（类型/含义/边界/枚举/格式/组合），禁止无意义 mock 占位 | P1 |
