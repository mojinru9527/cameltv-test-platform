...
overview = [
    ...
    N("P1 级：执行记录双轨/环依赖lazy压制/UI执行三入口/6套认领队列/cachedGet被signal绕过/4处useEffect无cleanup/文档3处硬伤（local-setup.md引用路径歧义等）/统计口径分裂"),
    N("主链路状态：需求✅→用例✅→计划✅(编排36.8%)→执行⚠️(通过率22.1%)→缺陷✅(删除为裸图标按钮a11y差)→报告⚠️(27.1%均8/12生成)→统计❌(口径矛盾)→通知⚠️(渠道2条但外部集成0)"),
...
req_flow = [
    ...
    N("问题（173）：①需求覆盖率 0%（页面）vs 33.3%（追溯）vs 67%（集成页）三口径；②交互缺口2217弱分页（50/页×45页，无跳页、URL未解码、首页被'首页'入口占满，无P0优先——UI报告修正：非无分页）；③蓝湖失败任务#30/#29/#26无清理入口；④用例内容渲染bug影响AI生成用例可读性（数字拆分）", "UI-05修正/UI-08/统计矛盾"),
]
...
# ============ 4 质量域 ============
quality_flow = [
    N("功能流转：执行结果 → 缺陷(8: 3致命/1严重P1/4) → 状态机(open→confirmed→fixing→pending_review→closed/rejected) → 报告(5份) → 追溯(链路口径) → 定时任务(4) → 通知(邮件+飞书webhook)", "实测：缺陷新建POST200✅/详情✅/编辑✅；删除为裸图标按钮（a11y不可见，能力存在）；缺陷删除后知识切片残留(deprecated)"),
    ...
    N("问题（173）：①统计口径5套并存（工作台9.1% vs 追溯22.1% vs 报告27.1% vs 集成页67%）；②缺陷删除按钮为无文本裸图标（a11y不可见，能力存在——UI报告修正：非无入口）；③缺陷删除级联不彻底（知识切片残留status=deprecated）；④报告5份全在8/12生成——无日常自动生成；⑤报告模板列全为'—'；⑥定时任务4个（2个执行中1个停用1个API回归）", "UI-04/UI-09/统计矛盾"),
]
...
platform_flow = [
    ...
    N("问题（173）：①前端删除入口问题：缺陷为裸图标按钮（a11y不可见）、蓝湖任务无删除（仅重试）；②环境页长变量值溢出无换行；③角色/邀请码 tab 初始空白无骨架屏；④DSH任务页'服务未启用'仍展示新建入口；⑤API Token掩码显示'****F4PY'（仅尾部4位可见，设计合理✅）；⑥权限码双份维护无CI校验", "UI-07/UI-10/UI-11/ARCH-11"),
]
...
issues = [
    ...
    N("P1-6 文档硬伤：docs/local-setup.md引用路径歧义（文件实存仓库根docs/，从test-platform-v2/docs/解析失效）+手册声称special/perftest可用但路由已隐藏+完整PRD技术栈过时(React18.3 vs 19.2.8)", "手册:11,51/路由index.tsx:214-215,234-235/完整PRD:71,100"),
    N("P1-7 缺陷删除按钮为无文本裸图标（a11y不可见）+删除后知识切片残留", "DefectTable.tsx:151-176裸图标；删除后knowledge source id=113 status=deprecated残留"),
