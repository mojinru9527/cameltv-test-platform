...
6. **C110-1/C110-2/C110-5 闭环**：wiki 基线（bundle#4/43 raw sources/158 页/10 diff）+ capture 复验（code 0/id=15/sources 16）
   + konfi 实测（29 配置表）+ 运营后台生产菜单 15 模块（系统模块按要求跳过）。
...
- C110-5（P2）：✅ 已闭环（konfi 29 配置表实测 + 运营后台 15 模块生产菜单）；C95-1/C74-2 Test5 契约补拉继续 Deferred。
