...
可追溯输入；`PARTIAL` 表示当前输入已可执行但只完成部分成功标准；`WAIVED`
表示风险负责人接受未验证风险，不等于测试通过。
...
| `B56-B08` | P0 | `WAIVED` | `B56-R0-LEGACY-PG`、`R0-LEGACY-PG` | 无旧 PostgreSQL 脱敏快照；开发于 2026-07-30 接受“旧库升级兼容性未经真实快照验证”的剩余风险 | 开发 / QA | 不记为 `PASS`；报告持续展示风险；未来获得合规快照或执行真实旧库迁移前，豁免自动失效并补测 |
| `B56-B09` | P0 | `PARTIAL` | `B56-R0-USER-DESIGN`、`B56-R0-ADMIN-DESIGN` | APP_UI 241 页、WEB_UI 102 页的项目权限和页面树已复核；三个用户端 PC 路由均映射到设计节点。运营后台设计源、精确版本和资产 SHA 仍缺 | 产品/设计负责人 / QA | 补充或确认当前运营后台设计源；固定两端版本、采集时间和资产 SHA-256，并完成 PC 页面视觉差异表 |
...
- `B56-B08` 按正式风险接受记为 `WAIVED`，不计作测试通过；其余条目仍未关闭。
  没有一项因本地测试、HTTP 200、mock、历史报告或代码存在而自动关闭。
