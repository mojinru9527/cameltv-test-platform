...
| Android 滚动场景 fps | 静态页 fps=0 | 滚动/播放期间采样到 fps>0（≥1 个样本） |
| Android 报告 | 已有 | 会话完成 + 报告统计（fps 不再恒 0） |
| iOS 采集 | 未执行 | 若驱动可用：tidevice 识别 → 会话 E2E；不可用则如实登记阻塞原因 |
| 证据 | — | `real-device-collection-batch99.json` + 截图/日志 |

## 3. 用户故事 + 验收标准

- As a **性能验收负责人**, I want Android 滚动/播放场景真实 fps 采样，so that C84-2 有据关闭。
