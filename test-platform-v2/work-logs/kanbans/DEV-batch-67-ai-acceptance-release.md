...
```
Batch 67 — 前置条件收口
├── 已完成: 清单 2.x/6.1 状态登记、六部门工件、看板
├── 🔄 进行中: QA 自测（AI Key 401 已记录为 B67-Q1）
├── ⏳ 待审批: push 授权 → Draft PR → 首轮 checks → 用户二次确认
└── ⏳ 下一步: 用户换新 AI Key（C67-1）+ Railway URL（C67-2）
```
...
| 阻塞项 | 严重度 | 描述 | 需要谁 | 记录时间 |
|--------|:------:|------|--------|----------|
| B67-Q1 | P1 | AI_API_KEY 实测 401，AI 验收无法解锁 | 用户换新 Key | 2026-08-02 |
| 6.1 | P1 | 部署服务器 URL 未提供（Railway 重试待操作） | 用户 | 2026-08-02 |
