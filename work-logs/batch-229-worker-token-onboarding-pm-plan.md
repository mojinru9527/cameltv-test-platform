...
### [ ] Task 1：Worker Token 鉴权契约（45 分钟）
...
### [ ] Task 3：一次性 Worker 配置与撤销入口（45 分钟）
...
### [ ] Task 4：Runbook 与启动器 fail-fast（30 分钟）
...
- [ ] 后端定向 Pytest + F821 + app 导入 + Alembic 单头/revision
- [ ] 前端相关 Vitest + typecheck + lint + build
- [ ] 后端/前端全量回归无新增失败
- [ ] `scan-common-bugs.ps1` 与 `dev-gate.ps1` 无 HARD
- [ ] 1440x900、768x1024、390x844 三视口关键路径
- [ ] Network 中相关 GET 每轮一次，控制台无错误
- [ ] 仓库、工件、截图无真实 Token/Cookie/密码
