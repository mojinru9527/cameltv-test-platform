...
- AI 调试抽屉在每条真实操作下展示：输入 Token、缓存命中 Token、未命中 Token、输出 Token、缓存命中率、应用内复用次数。
- `0%` 是供应商明确返回零命中；`—` 是历史记录或供应商没有返回缓存明细，两者不得混淆。
- 不展示原始 Prompt、动态需求内容、API Key、`prompt_cache_key` 或未经配置验证的金额估算。
- 保持现有右侧 Sheet、语义 Badge 和紧凑 `dl`，移动端占满可用宽度并纵向滚动，不新建统计卡或图表。
