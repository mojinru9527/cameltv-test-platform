...
| 覆盖 | 良 | 后端 94 例 + 前端 721 例 + 端到端演练 25/25（含失败用例证据）；缺口是 Test5 真机（C258-1） |
...
6. **演练必须包含失败用例**：DoD 写明「失败用例有截图/请求回放」，只跑通过用例证明不了这一条。补了故意失败的 API/Web 各 1 条，验证任务结论为 `failed`、证据留存、且未被伪造成通过（19→25 项检查）。
