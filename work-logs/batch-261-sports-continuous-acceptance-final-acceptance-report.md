...
| 6 | 「改了 X 要跑哪些」可用 | ✅ **达成**（时延项未实测） | Batch 260 §B3-3（API + 前端视图；防 N+1 有断言，≤2s 需真实数据测） |
...
**未实测**：DoD 的「≤2s」是**墙钟指标**，本机库无数据无法测真实时延；已用"查询条数不随规模增长"作为可执行替代保证，真实时延需在有数据的 gr环境测一次（并入 C261-1 的验证范围）。
