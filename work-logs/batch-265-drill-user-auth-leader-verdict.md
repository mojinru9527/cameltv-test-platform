...
- **C265-3（P1）**：**试点集执行 payload 不带可执行细节**——API 侧 50/50 是裸 `GET <base>/`（404），Web 侧 `steps=[]` 且 30 张截图逐字节相同（空白页），故「30/30 通过」属**空过**。本判决同时作废本批早期"Web 30/30 在 Test5 通过"的表述。解除条件=payload 携带完整用例定义（或节点按 case id 取全量定义）后重跑 3 版本并达 `meets_all=true`。
