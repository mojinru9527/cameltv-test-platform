...
**C266-4 收尾尝试**：首页类用例统一改为「先 `wait_visible h1`（30s 锚点）+ 2.5s 稳定等待 → `expect_visible`（任一匹配可见）断言标签」，部分复跑结果为 Web **27/30、29/30**（第三版被本机连接中断）；有改善但**尚未稳定**，`C266-4` 保持 Open 并细化了消除条件。
