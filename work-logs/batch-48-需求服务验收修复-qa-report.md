...
- 后端全新检出连续两轮均为 819 项中 818 通过、1 失败：首轮缺少 `fastmcp`，补充后第二轮继续暴露 `beautifulsoup4` 未声明。根因是后端 Provider 直接导入固定子模块，但部署清单未同步其完整运行依赖；现已把 `lanhu-mcp/requirements.txt` 对应依赖组显式同步到后端 `requirements.txt`。
