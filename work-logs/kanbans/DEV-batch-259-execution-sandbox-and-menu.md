...
| 3 | dry-run≠沙箱 + 计划路径 validate（B2-3） | ✅ | 🔄 ⬅️ | ⏳ | ⏳ | ⏳ | **当前位置** |
...
```
Batch 259 — Slice 3：纠正「dry-run = 沙箱」+ 计划路径不再跳过校验
├── 已完成: Slice 1（S4 关闭）、Slice 2（S5 关闭）；工件已提交
│          commit 5312b497 / 6c4b7942 / 9da274f8
├── 🔄 进行中: case_compiler 文档/注释纠正 + test_plan_service 去掉 validate=False
├── ⏳ 待审批: 本批次一次总确认（推送 + Draft PR + required checks 通过后合入）
└── ⏳ 下一步: Slice 4（危险 API 拦截）→ Slice 5（沙箱硬化 H1）→ Slice 6（菜单收敛）→ QA/Leader
```
