...
| 3 | 「改了 X 要跑哪些」查询（B3-3） | ✅ | 🔄 ⬅️ | ⏳ | ⏳ | ⏳ | **当前位置**，拼接既有四段 |
...
```
Batch 260 — Slice 3：「改了 X 要跑哪些」查询
├── 已完成: Slice 1（ImpactEdge）、Slice 2（关联构建 + 覆盖率 + 回填脚本）
│          commit b8e3bb32 / 09fa9a7a / 64787ae4 / 249e0632
├── 🔄 进行中: impact_query_service + /api/v1/impact 查询（拼接既有四段，≤2s，防 N+1）
├── 未决: C260-1（真实体育资产上的覆盖率数字需在目标环境跑回填脚本产出；本机库无该数据）
├── ⏳ 待审批: 本批次一次总确认（推送 + Draft PR + required checks 通过后合入）
└── ⏳ 下一步: Slice 4（复用命中率埋点）→ Slice 5（Tab 收敛）→ QA/Leader
```
