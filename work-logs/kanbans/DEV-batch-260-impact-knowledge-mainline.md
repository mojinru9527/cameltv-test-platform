...
|---|-------|:----:|:----:|:----:|:----:|:----:|------|
| 1 | `ImpactEdge` 模型 + 迁移（B3-1） | ✅ | 🔄 ⬅️ | ⏳ | ⏳ | ⏳ | **当前位置**（数据模型变更） |
| 2 | 关联构建 + 覆盖率证据（B3-2） | ✅ | ⏳ | ⏳ | ⏳ | ⏳ | 复用既有 linker |
| 3 | 「改了 X 要跑哪些」查询（B3-3） | ✅ | ⏳ | ⏳ | ⏳ | ⏳ | 拼接既有四段 |
| 4 | 复用建议命中率埋点（B3-4） | ✅ | ⏳ | ⏳ | ⏳ | ⏳ | 主体已存在，只补埋点 |
| 5 | 知识中心 Tab 收敛 ≤3（B3-5） | ✅ | ⏳ | ⏳ | ⏳ | ⏳ | 前端 |

## 📍 当前位置

```
Batch 260 — Slice 1：ImpactEdge 模型 + 迁移
├── 已完成: worktree 验证（base=573f5c12）；PRD/PM/Design 工件；B3 开工前侦察（4 项，已改写批次形态）
├── 🔄 进行中: impact_edge 模型 + Alembic 迁移 + 幂等/kind 校验
├── ⏳ 待审批: 本批次一次总确认（推送 + Draft PR + required checks 通过后合入）
└── ⏳ 下一步: Slice 2（关联构建）→ Slice 3（查询）→ Slice 4（命中率）→ Slice 5（Tab）→ QA/Leader
```
