...
| 2 | 关联构建 + 覆盖率证据（B3-2） | ✅ | 🔄 ⬅️ | ⏳ | ⏳ | ⏳ | **当前位置**，复用既有 linker |
...
```
Batch 260 — Slice 2：关联构建（需求变更 → 模块 → 用例）
├── 已完成: Slice 1（ImpactEdge 模型 + 迁移 + 幂等 upsert）；PRD/PM/Design 工件；开工前侦察
│          commit b8e3bb32 / 09fa9a7a
├── 🔄 进行中: impact_graph_service.build_edges + 回填脚本 + 模块关联覆盖率证据
├── ⏳ 待审批: 本批次一次总确认（推送 + Draft PR + required checks 通过后合入）
└── ⏳ 下一步: Slice 3（查询 API）→ Slice 4（复用命中率）→ Slice 5（Tab 收敛）→ QA/Leader
```
