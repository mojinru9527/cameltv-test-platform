...
| 3 | 试点数据集与基线制备（B4-2） | ✅ | ✅ | ✅ | ⏳ | ⏳ | 9 例；脚本 exit 3 如实报缺口 |
| 4 | 连续 3 版本演练 + 模板（B4-3/4/5） | ✅ | 🔄 ⬅️ | ⏳ | ⏳ | ⏳ | **当前位置**，真实数字需 VPN → C261-1 |
| 5 | §5 九条验收报告（B4-5 收尾） | ✅ | ⏳ | ⏳ | ⏳ | ⏳ | |

## 📍 当前位置

```
Batch 261 — Slice 4：连续 3 版本演练 + 验收报告模板（B4-3/4/5）
├── 已完成: Slice 1/2（B4-1 证据包定型）、Slice 3（B4-2 数据集与基线）
│          commit c2e256c9 / eb4a750a / 173bb15b / 3c997366
├── 🔄 进行中: 一条命令跑 3 个版本的演练（前置检查 + 环境不可达时明确失败）+ 报告模板
├── ⏳ 待审批: 本批次一次总确认（推送 + Draft PR + required checks 通过后合入）
└── ⏳ 下一步: Slice 5（§5 九条验收报告，环境依赖项标注 C 条件）→ QA/Leader → 一次总确认
```
