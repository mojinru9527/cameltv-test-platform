...
|---|-------|:----:|:----:|:----:|:----:|:----:|------|
| 1 | 证据包校验服务（B4-1 后端） | ✅ | 🔄 ⬅️ | ⏳ | ⏳ | ⏳ | **当前位置** |
| 2 | 校验 API + 篡改显红前端（B4-1） | ✅ | ⏳ | ⏳ | ⏳ | ⏳ | |
| 3 | 试点数据集与基线制备（B4-2） | ✅ | ⏳ | ⏳ | ⏳ | ⏳ | 真实导入需环境 → C261-1 |
...
```
Batch 261 — Slice 1：证据包校验服务
├── 已完成: worktree 验证（base=3788c66d）；PRD/PM/Design 工件；B4 开工前侦察（4 项）
├── 🔄 进行中: evidence_bundle_service.verify_bundle（篡改/缺失/多余 + 复用既有完整性策略）
├── ⏳ 待审批: 本批次一次总确认（推送 + Draft PR + required checks 通过后合入）
└── ⏳ 下一步: Slice 2（API + 前端显红）→ Slice 3（数据集/基线制备）→ Slice 4（3 版本演练）→ Slice 5（九条报告）
```
